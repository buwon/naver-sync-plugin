...
declare namespace Electron {
	...
	interface App extends NodeJS.EventEmitter {
		...
		setClientCertRequestPasswordHandler(
			handler: (
				clientCertRequestParams: ClientCertRequestParams
			) => Promise<string>
		): void;
		/**
	...
	class BaseWindow extends NodeEventEmitter {
		...
		on(
			...
			listener: (event: WindowSessionEndEvent) => void
		): this;
		/**
		 * @platform win32
		 */
	...
	class IpcMainServiceWorker extends NodeEventEmitter {
		...
		on(
			channel: string,
			listener: (event: IpcMainServiceWorkerEvent, ...args: any[]) => void
		): this;
		/**
		 * Adds a one time `listener` function for the event. This `listener` is invoked
		 * only the next time a message is sent to `channel`, after which it is removed.
	...
	interface PowerMonitor extends NodeJS.EventEmitter {
		 ...
		 */
		on(
			event: "speed-limit-change",
			listener: (
				details: Event<PowerMonitorSpeedLimitChangeEventParams>
			) => void
		): this;
		/**
		 * @platform darwin,win32
		...
		on(
			...
			listener: (
				details: Event<PowerMonitorThermalStateChangeEventParams>
			) => void
		): this;
		/**
		 * @platform darwin
	...
	interface Protocol {
		...
		interceptStreamProtocol(
			...
			handler: (
				request: ProtocolRequest,
				callback: (
					response: NodeJS.ReadableStream | ProtocolResponse
				) => void
			) => void
		): boolean;
		/**
	...
	interface PushNotifications extends NodeJS.EventEmitter {
		...
		on(
			...
			listener: (event: Event, userInfo: Record<string, any>) => void
		): this;
		/**
		 * @platform darwin
		 */
	...
	class ServiceWorkers extends NodeEventEmitter {
		...
		on(
			...
			listener: (
				details: Event<ServiceWorkersRunningStatusChangedEventParams>
			) => void
		): this;
		/**
		 * @experimental
	...
	class Session extends NodeEventEmitter {
		...
		on(
			...
			listener: (event: Event, details: HidDeviceAddedDetails) => void
		): this;
		off(
			event: "hid-device-added",
			listener: (event: Event, details: HidDeviceAddedDetails) => void
		...
		on(
			...
			listener: (event: Event, details: HidDeviceRemovedDetails) => void
		): this;
		off(
			event: "hid-device-removed",
			listener: (event: Event, details: HidDeviceRemovedDetails) => void
		...
		on(
			...
			listener: (event: Event, details: HidDeviceRevokedDetails) => void
		): this;
		off(
			event: "hid-device-revoked",
			listener: (event: Event, details: HidDeviceRevokedDetails) => void
		...
		on(
			...
			listener: (event: Event, details: SerialPortRevokedDetails) => void
		): this;
		off(
			event: "serial-port-revoked",
			listener: (event: Event, details: SerialPortRevokedDetails) => void
		...
		on(
			...
			listener: (event: Event, details: UsbDeviceRevokedDetails) => void
		): this;
		off(
			event: "usb-device-revoked",
			listener: (event: Event, details: UsbDeviceRevokedDetails) => void
		...
		createInterruptedDownload(
			options: CreateInterruptedDownloadOptions
		): void;
		/**
		...
		): Promise<Electron.SharedDictionaryInfo[]>;
		/**
		 * an array of shared dictionary information entries in Chromium's networking
		 * service's storage.
		 *
		 ...
		 */
		getSharedDictionaryUsageInfo(): Promise<
			Electron.SharedDictionaryUsageInfo[]
		>;
		/**
		...
		setUSBProtectedClassesHandler(
			handler:
				| ((
						details: USBProtectedClassesHandlerHandlerDetails
				  ) => string[])
				| null
		): void;
		/**
	...
	interface SystemPreferences extends NodeJS.EventEmitter {
		 ...
		 * @platform darwin
		 */
		getUserDefault<Type extends keyof UserDefaultTypes>(
			key: string,
			type: Type
		 ...
		 * @platform darwin
		 */
		setUserDefault<Type extends keyof UserDefaultTypes>(
			key: string,
			type: Type,
	...
	class WebContents extends NodeEventEmitter {
		...
		on(
			...
			listener: (event: Event, mouse: MouseInputEvent) => void
		): this;
		off(
			event: "before-mouse-event",
			listener: (event: Event, mouse: MouseInputEvent) => void
		...
		on(
			event: "frame-created",
			listener: (event: Event, details: FrameCreatedDetails) => void
		): this;
		off(
			event: "frame-created",
			listener: (event: Event, details: FrameCreatedDetails) => void
		...
		on(
			event: "paint",
			listener: (
				details: Event<WebContentsPaintEventParams>,
				dirtyRect: Rectangle,
				/**
		...
		onBeforeSendHeaders(
			...
			listener:
				| ((
						...
						callback: (
							beforeSendResponse: BeforeSendResponse
						) => void
				  ) => void)
				| null
		): void;
	...
	interface WebviewTag extends HTMLElement {
		...
		addEventListener(
			event: "devtools-open-url",
			listener: (event: DevtoolsOpenUrlEvent) => void,
			useCapture?: boolean
		): this;
		removeEventListener(
			event: "devtools-open-url",
		...
		addEventListener(
			event: "devtools-search-query",
			listener: (event: DevtoolsSearchQueryEvent) => void,
			useCapture?: boolean
		): this;
		removeEventListener(
			event: "devtools-search-query",
	...
	namespace Common {
		...
		type ClearSharedDictionaryCacheForIsolationKeyOptions =
			Electron.ClearSharedDictionaryCacheForIsolationKeyOptions;
		...
		type ConfigureHostResolverOptions =
			Electron.ConfigureHostResolverOptions;
		...
		type DisplayMediaRequestHandlerOpts =
		...
		type FileSystemAccessRestrictedDetails =
			Electron.FileSystemAccessRestrictedDetails;
		...
		type LoginAuthenticationResponseDetails =
			Electron.LoginAuthenticationResponseDetails;
		...
		type PowerMonitorSpeedLimitChangeEventParams =
			Electron.PowerMonitorSpeedLimitChangeEventParams;
		type PowerMonitorThermalStateChangeEventParams =
			Electron.PowerMonitorThermalStateChangeEventParams;
		...
		type WebContentsConsoleMessageEventParams =
			Electron.WebContentsConsoleMessageEventParams;
...
declare namespace Remote {
	...
	const globalShortcut: GlobalShortcut;
	type GlobalShortcut = Electron.GlobalShortcut;
	const nativeImage: typeof Electron.NativeImage;
	type NativeImage = Electron.NativeImage;
	const screen: Screen;
	type Screen = Electron.Screen;
	const shell: Shell;
	...
	function getCurrentWebContents(): WebContents;
